...
(function() {
  ...
  describe('ProxyFrame', function() {
    before(function() {
      return this.queueSpy = sinon.spy();
    });
    it('creates a new iframe', function() {
      window.pf = this.proxyFrame = new ProxyFrame({
        host: "" + location.protocol + "//" + location.host,
        path: '/test/helpers/proxy'
      });
      expect(this.proxyFrame.el).to.match('.proxy-frame');
    ...
    describe('when the back end is available', function() {
      ...
      it('resolves deferreds in the queue', function(done) {
        return this.proxyFrame.deferreds[this.inQueue.id].done(function() {
          return done();
        });
      });
      return it('sends a request and receives a response', function(done) {
    ...
    return describe('when the back end is unavailable', function() {
      ...
      return it('rejects attempted sends', function(done) {
        this.badProxyFrame.one('response', function(e, response) {});
